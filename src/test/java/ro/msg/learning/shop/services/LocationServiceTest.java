...
public class LocationServiceTest {
    ...
    public void setUp() {
        ...
        StockQuantityProductWrapper stockQuantityProductWrapper = new StockQuantityProductWrapper(stock, 5, 5);
        List<StockQuantityProductWrapper> stockQuantityProductWrappers = new ArrayList<>();
        stockQuantityProductWrappers.add(stockQuantityProductWrapper);
        when(locationStrategy.getStockQuantityProductWrapper(any())).thenReturn(stockQuantityProductWrappers);
    }

    private OrderDtoIn getOrderDtoIn() {
        OrderDtoIn orderDtoIn = new OrderDtoIn();
        orderDtoIn.setOrderDetails(getOrderDetailsDtoList());
        return orderDtoIn;
    }
    ...
    public void getLocationsForOrderTest() {
        ...
        locationService.getLocationsForOrder(getOrderDtoIn()).
            parallelStream().
            forEach(location1 -> {
